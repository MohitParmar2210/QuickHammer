...
export const endedAuctionCron = () => {
  cron.schedule("*/1 * * * *", async () => {
    //*=minutes *=hours *=days *=months *=years */1=it means code in corn executes after one minutes
    const now = new Date();
    console.log("Cron for ended auction running...");
    const endedAuctions = await Auction.find({
      endTime: { $lt: now },
      commissionCalculated: false,
    }); //jo auction complete ho chuka hai
    for (const auction of endedAuctions) {
      try {
        ...
        auctioneer.unpaidCommission = commissionAmount; //add the commission ammount in unpaid commission if any unpaid commission is there so not allowed then for any  other auction create
        if (highestBidder) {
          ...
          await User.findByIdAndUpdate(
            bidder._id,
            {
              $inc: {
                ...
                auctionsWon: 1,
              }, //give info about money spend and number of auction won by bidder
            },
            { new: true }
          );
          await User.findByIdAndUpdate(
            ...
            }, //commision by auctioneer to super admin
            { new: true }
          );
          ...
          await auction.save();//if highest bidder not found
        }
      } catch (error) {
