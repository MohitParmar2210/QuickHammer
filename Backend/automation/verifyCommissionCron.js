...
export const verifyCommissionCron = () => {
  cron.schedule("*/1 * * * *", async () => {// check in every minutes that which user commission payment is approved and then use us
    console.log("Running Verify Commission Cron...");
    const approvedProofs = await PaymentProof.find({ status: "Approved" });//take all approved payment from database
    for (const proof of approvedProofs) {
      try {
        const user = await User.findById(proof.userId);//find the user 
        let updatedUserData = {};
        if (user) {
...
//these code check in every minutes that which user commission payment is approved and then use us
//then addjust a commission 
//update payment status
//send email to user that their payment is settled now